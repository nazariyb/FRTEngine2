...
struct CD3DX12_VIEWPORT : public D3D12_VIEWPORT
{
		...
		FLOAT maxDepth = D3D12_MAX_DEPTH)
	{
		...
		{
			case D3D12_RESOURCE_DIMENSION_BUFFER: TopLeftX = topLeftX;
				TopLeftY = 0.0f;
				Width = Desc.Width - topLeftX;
				Height = 1.0f;
				break;
			case D3D12_RESOURCE_DIMENSION_TEXTURE1D: TopLeftX = topLeftX;
				TopLeftY = 0.0f;
				Width = (SubresourceWidth ? SubresourceWidth : 1.0f) - topLeftX;
				Height = 1.0f;
				break;
			case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
			case D3D12_RESOURCE_DIMENSION_TEXTURE3D: TopLeftX = topLeftX;
				TopLeftY = topLeftY;
				Width = (SubresourceWidth ? SubresourceWidth : 1.0f) - topLeftX;
...
struct CD3DX12_TEXTURE_COPY_LOCATION : public D3D12_TEXTURE_COPY_LOCATION
{
	...
	CD3DX12_TEXTURE_COPY_LOCATION (_In_ ID3D12Resource* pRes, const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& Footprint)
	{
		pResource = pRes;
...
struct CD3DX12_DESCRIPTOR_RANGE1 : public D3D12_DESCRIPTOR_RANGE1
{
	...
	{
		Init(
			*this, rangeType, numDescriptors, baseShaderRegister, registerSpace, flags,
...
struct CD3DX12_RESOURCE_DESC : public D3D12_RESOURCE_DESC
{
		...
		D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE)
	{
		return CD3DX12_RESOURCE_DESC(
			D3D12_RESOURCE_DIMENSION_BUFFER, resAllocInfo.Alignment, resAllocInfo.SizeInBytes,
	...
	_In_reads_(NumSubresources) const D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	for (UINT i = 0; i < NumSubresources; ++i)
	{
		...
		MemcpySubresource(
			&DestData, &pSrcData[i], static_cast<SIZE_T>(pRowSizesInBytes[i]), pNumRows[i],
	...
	_In_reads_(NumSubresources) D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	pDestinationResource->GetDevice(__uuidof(*pDevice), reinterpret_cast<void**>(&pDevice));
	pDevice->GetCopyableFootprints(
		&Desc, FirstSubresource, NumSubresources, IntermediateOffset, pLayouts, pNumRows, pRowSizesInBytes,
		&RequiredSize);
	pDevice->Release();
	...
	_In_reads_(NumSubresources) D3D12_SUBRESOURCE_DATA* pSrcData)
{
	...
	return UpdateSubresources(
		pCmdList, pDestinationResource, pIntermediate, FirstSubresource, NumSubresources, RequiredSize, Layouts,
	...
	_Always_(_Outptr_opt_result_maybenull_) ID3DBlob** ppErrorBlob)
{
	...
	switch (MaxVersion)
	{
		case D3D_ROOT_SIGNATURE_VERSION_1_0: switch (pRootSignatureDesc->Version)
			{
				case D3D_ROOT_SIGNATURE_VERSION_1_0: return D3D12SerializeRootSignature(
					...
					if (SUCCEEDED(hr))
					{
						for (UINT n = 0; n < desc_1_1.NumParameters; n++)
						{
							...
							switch (desc_1_1.pParameters[n].ParameterType)
							{
								...
								case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE: const D3D12_ROOT_DESCRIPTOR_TABLE1&
									...
									auto pDescriptorRanges_1_0 = reinterpret_cast<D3D12_DESCRIPTOR_RANGE*>(
										pDescriptorRanges);
									...
									if (SUCCEEDED(hr))
									{
										for (UINT x = 0; x < table_1_1.NumDescriptorRanges; x++)
										{
											...
											pDescriptorRanges_1_0[x].RangeType = table_1_1.pDescriptorRanges[x].
												RangeType;
		...
		case D3D_ROOT_SIGNATURE_VERSION_1_1: return D3D12SerializeVersionedRootSignature(
...
struct CD3DX12_PIPELINE_STATE_STREAM_PARSE_HELPER : public ID3DX12PipelineParserCallbacks
{
	...
	virtual void FlagsCb (D3D12_PIPELINE_STATE_FLAGS Flags) override { PipelineStream.Flags = Flags; }
	virtual void NodeMaskCb (UINT NodeMask) override { PipelineStream.NodeMask = NodeMask; }

	virtual void RootSignatureCb (ID3D12RootSignature* pRootSignature) override
	{
		PipelineStream.pRootSignature = pRootSignature;
	}

	virtual void InputLayoutCb (const D3D12_INPUT_LAYOUT_DESC& InputLayout) override
	{
		PipelineStream.InputLayout = InputLayout;
	}

	virtual void IBStripCutValueCb (D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBStripCutValue) override
	{
		PipelineStream.IBStripCutValue = IBStripCutValue;
	}

	virtual void PrimitiveTopologyTypeCb (D3D12_PRIMITIVE_TOPOLOGY_TYPE PrimitiveTopologyType) override
	{
		PipelineStream.PrimitiveTopologyType = PrimitiveTopologyType;
	}

	virtual void VSCb (const D3D12_SHADER_BYTECODE& VS) override { PipelineStream.VS = VS; }
	virtual void GSCb (const D3D12_SHADER_BYTECODE& GS) override { PipelineStream.GS = GS; }

	virtual void StreamOutputCb (const D3D12_STREAM_OUTPUT_DESC& StreamOutput) override
	{
		PipelineStream.StreamOutput = StreamOutput;
	}

	virtual void HSCb (const D3D12_SHADER_BYTECODE& HS) override { PipelineStream.HS = HS; }
	virtual void DSCb (const D3D12_SHADER_BYTECODE& DS) override { PipelineStream.DS = DS; }
	virtual void PSCb (const D3D12_SHADER_BYTECODE& PS) override { PipelineStream.PS = PS; }
	virtual void CSCb (const D3D12_SHADER_BYTECODE& CS) override { PipelineStream.CS = CS; }

	virtual void BlendStateCb (const D3D12_BLEND_DESC& BlendState) override
	{
		PipelineStream.BlendState = CD3DX12_BLEND_DESC(BlendState);
	}

	virtual void DepthStencilStateCb (const D3D12_DEPTH_STENCIL_DESC& DepthStencilState) override
	{
		PipelineStream.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC1(DepthStencilState);
		SeenDSS = true;
	}

	virtual void DepthStencilState1Cb (const D3D12_DEPTH_STENCIL_DESC1& DepthStencilState) override
	{
		PipelineStream.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC1(DepthStencilState);
		SeenDSS = true;
	}

	virtual void DSVFormatCb (DXGI_FORMAT DSVFormat) override
	{
	...
	virtual void RasterizerStateCb (const D3D12_RASTERIZER_DESC& RasterizerState) override
	{
		PipelineStream.RasterizerState = CD3DX12_RASTERIZER_DESC(RasterizerState);
	}

	virtual void RTVFormatsCb (const D3D12_RT_FORMAT_ARRAY& RTVFormats) override
	{
		PipelineStream.RTVFormats = RTVFormats;
	}

	virtual void SampleDescCb (const DXGI_SAMPLE_DESC& SampleDesc) override { PipelineStream.SampleDesc = SampleDesc; }
	virtual void SampleMaskCb (UINT SampleMask) override { PipelineStream.SampleMask = SampleMask; }

	virtual void ViewInstancingCb (const D3D12_VIEW_INSTANCING_DESC& ViewInstancingDesc) override
	{
		PipelineStream.ViewInstancingDesc = CD3DX12_VIEW_INSTANCING_DESC(ViewInstancingDesc);
	}

	virtual void CachedPSOCb (const D3D12_CACHED_PIPELINE_STATE& CachedPSO) override
	{
	...
	D3D12_PIPELINE_STATE_SUBOBJECT_TYPE SubobjectType)
{
	...
	{
		case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL1: return
	...
	ID3DX12PipelineParserCallbacks* pCallbacks)
{
	...
	for (SIZE_T CurOffset = 0, SizeOfSubobject = 0; CurOffset < Desc.SizeInBytes; CurOffset += SizeOfSubobject)
	{
		...
		switch (SubobjectType)
		{
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE: pCallbacks->RootSignatureCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_VS: pCallbacks->VSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PS: pCallbacks->PSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DS: pCallbacks->DSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_HS: pCallbacks->HSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_GS: pCallbacks->GSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_CS: pCallbacks->CSCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_STREAM_OUTPUT: pCallbacks->StreamOutputCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_BLEND: pCallbacks->BlendStateCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_MASK: pCallbacks->SampleMaskCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RASTERIZER: pCallbacks->RasterizerStateCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL: pCallbacks->DepthStencilStateCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL1: pCallbacks->DepthStencilState1Cb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_INPUT_LAYOUT: pCallbacks->InputLayoutCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_IB_STRIP_CUT_VALUE: pCallbacks->IBStripCutValueCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY: pCallbacks->PrimitiveTopologyTypeCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS: pCallbacks->RTVFormatsCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT: pCallbacks->DSVFormatCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_DESC: pCallbacks->SampleDescCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_NODE_MASK: pCallbacks->NodeMaskCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_CACHED_PSO: pCallbacks->CachedPSOCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_FLAGS: pCallbacks->FlagsCb(
				...
				break;
			case D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_VIEW_INSTANCING: pCallbacks->ViewInstancingCb(
				...
				break;
			default: pCallbacks->ErrorUnknownSubobject(SubobjectType);
				return E_INVALIDARG;
				break;
		}
	}
...
private:
	...
	using SUBOBJECT_WRAPPER = struct SUBOBJECT_WRAPPER : public D3D12_STATE_SUBOBJECT
	{
...
public:
	...
	virtual D3D12_STATE_SUBOBJECT_TYPE Type () const override
	{
...
private:
	...
	virtual void* Data () override { return &m_Desc; }
...
private:
	...
	virtual void* Data () override { return m_pRootSig.GetAddressOf(); }
